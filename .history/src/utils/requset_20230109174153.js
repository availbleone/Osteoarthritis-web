...
http.interceptors.request.use((config) => {
  ...
  if (config.url.indexOf("ljService") !== -1) {
...
http.interceptors.response.use((config) => {
  ...
  let msgs = new x2js().xml2js(config.data)
  console.log(msgs);
  console.log(msgs.Response.Code > '0');
  if (msgs.Response.Code != '0') {
    //后台响应了错误的结果
    MessageBox({
      // message: msgs['Response']['Desc'],
      // title: "错误提示",
      // type: "error",
    });
  }
